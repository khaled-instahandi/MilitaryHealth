...
namespace Api.Controllers
{
    ...
    public class DoctorsController : ControllerBase
    {
        ...
        {
            Expression<Func<Doctor, bool>>? filterExpr = null;

            if (!string.IsNullOrWhiteSpace(filter))
            {
                filterExpr = a => a.FullName.Contains(filter!) || a.Code.Contains(filter!);
            }

            var query = new GetEntitiesQuery<Doctor, DoctorDto>(
                filterExpr,
                null,
                sortBy,
                sortDesc,
                ...
                pageSize
                ,
                    new Expression<Func<Doctor, object>>[] { a => a.ContractType,b=>b.Specialization }
        ...
        {
            var query = new GetEntityByIdQuery<Doctor, DoctorDto>(id);
            var result = await _mediator.Send(query);
        ...
        public async Task<IActionResult> Put(int id, [FromBody] DoctorRequest dto)
        {
            ...
            var command = new UpdateEntityCommand<Doctor, DoctorRequest>(id, dto);
            var result = await _mediator.Send(command);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var command = new DeleteEntityCommand<Doctor>(id);
