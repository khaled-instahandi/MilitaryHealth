...
{
    [ApiController]
    [Route("api/InternalExams")]
    //[Authorize]
    public class InternalExamsController : ControllerBase
    {
        private readonly IMediator _mediator;

        public InternalExamsController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            Expression<Func<InternalExam, bool>>? filterExpr = null;
            ...
            var query = new GetEntitiesQuery<InternalExam, InternalExamDto>(
                filterExpr,
                null,
                sortBy,
                sortDesc,
                ...
                pageSize
                ,
                    new Expression<Func<InternalExam, object>>[] { b => b.Result, c => c.Doctor }
        ...
        {
            var query = new GetEntityByIdQuery<InternalExam, InternalExamDto>(id);
            var result = await _mediator.Send(query);
        ...
        // POST: api/Doctors
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] InternalExamRequest dto)
        {
            ...
            var command = new CreateEntityCommand<InternalExam, InternalExamRequest>(dto);
        ...
        // PUT: api/Doctors/5
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] InternalExamRequest dto)
        {
            if (!ModelState.IsValid)
            {
                var errors = ModelState
            ...
            var command = new UpdateEntityCommand<InternalExam, InternalExamRequest>(id, dto);
            var result = await _mediator.Send(command);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            var command = new DeleteEntityCommand<InternalExam>(id);
